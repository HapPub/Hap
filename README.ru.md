...
См. [матрицу возможностей](docs/CAPABILITIES.md): состояние реализации, проверенные хосты и известные ограничения. Команды инструментов хоста требуют Python 3.11+; сопряжение SSH также требует OpenSSH и OpenSSL.

В этой ветке исправлены пути SSH с пробелами и коды выхода при ошибках. Компоненты Cangjie проверяются во временном каталоге до публикации. Границы проверки указаны в матрице возможностей.
...
Обновлённый Hapup поддерживает [выбор маршрута загрузки](docs/INSTALLATION.md#download-routes): измерение скорости, ускорители для материкового Китая, переключение при низкой скорости и параметры `--region global|zh-cn` / `--route`. Используйте обновлённый скрипт; файлы прежних выпусков не изменяются.
...
Установка SDK выполняется на **нативном хосте macOS/Linux**. Semeru JDK также устанавливается на **Windows x64** с Python 3.11+ и возвращает команду активации PowerShell. Для остальных Windows SDK и
...
Обычные `get cangjie-sdk/stdx` устанавливают один компонент без активации, включая
проверенные nightly. `--legacy-plan` и явно заданные старые параметры recipe
сохраняют режим планирования. `--plan` сообщает об ожидающем
...
| --- | --- | --- |
| Установка SDK/JDK | Нативные macOS/Linux; Semeru JDK также на Windows x64, остальные Windows SDK и чужие хосты — только план | Доступность пакетов ниже не равна доступности бинарных файлов HapCLI. |
| IBM Semeru JDK | Установка из GitHub Releases; компиляция/запуск Java проверены на macOS ARM64 и Windows x64 | Нужен подходящий архив Open Edition JDK; совместимость со всеми проектами Gradle не гарантируется. |
...
| Windows AMD64 | Включён в обе сборки с фиксированным компилятором | Нужны нативные тесты и запуск из ZIP; Semeru JDK устанавливается нативно; остальные SDK остаются планами. |
...
## Движки установщиков и SSH в локальной сети

`hap get nsis|wix` устанавливает явно выбранный пакет с фиксированной контрольной суммой в приватный каталог. `hap get ssh` подготавливает клиент; сервер Windows требует явного выбора частного сетевого интерфейса. `hap ssh --pair` и `hap ssh conn` регистрируют отдельные ключи через TLS с привязкой сертификата и проверкой ключа SSH-сервера. Требования Python 3.11+, OpenSSH и OpenSSL, отзыв доступа и ограничения платформ описаны в [руководстве](docs/HOST_TOOLS.md).

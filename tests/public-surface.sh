...
# Check generated bytes against their reviewed plaintext sources before scanning.
# Compressed base64 can coincidentally contain device-ID patterns; its source
# files remain covered by the scan below.
python3 "$ROOT/scripts/embed-host-tools.py" --check
...
if grep -R -n -E \
  ...
  --exclude=host_tools_embedded.cj \
  --exclude='*.pyc' \
  --exclude-dir=.git \
  --exclude-dir=target \

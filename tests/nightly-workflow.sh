...
grep -q 'bootstrap-cangjie-from-mirror.sh' "$workflow" || fail_test "mirror bootstrap bridge is not used"
grep -q 'cangjie-sdk-resolution.json' "$workflow" || fail_test "SDK resolution receipt is not retained"
...
grep -q 'resolution_output=$(cygpath -u' "$ROOT/scripts/ci/bootstrap-cangjie-from-mirror.sh" || fail_test "Windows bootstrap receipt path normalization is missing"
